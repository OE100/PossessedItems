...
namespace PossessedItems.machines.impl.mask;
...
public class MaskStateManager : MonoBehaviour
{
    ...
    private void Update()
    {
        PossessedItemsBehaviour.Instance.SyncLocationServerRpc(_networkObject, 
    ...
    {
        PossessedItemsBehaviour.Instance.SetEyesFilledServerRpc(data.Mask.NetworkObject, false);
        ...
        if (Physics.Raycast(at.position, at.forward, out var hit, 5f, Physics.DefaultRaycastLayers))
            if (hit.transform.gameObject.TryGetComponent(out DoorLock doorLock))
                PossessedItemsBehaviour.Instance.StartCoroutine(DoorHandler(doorLock));
    ...
    private static State Wait(State previousState, Data data)
    {
        ...
        if (data.IsPrimed)
        {
            if (heldByPlayer && !data.Mask.attaching)
                PossessedItemsBehaviour.Instance.AttachServerRpc(data.Mask.NetworkObject);
            return State.Wait;
        }
